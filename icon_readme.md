...
To deal with blurred panel icons, increase the panel size up to 30px in `Systems Settings` â†’ `Panel` (see [screenshot](https://i.imgur.com/oToRBYv.png)).
</details>
...
Go to `Edit` â†’ `Preferences...`. Click on `Side Pane` tab. Under `Side Pane`, look for `Icon Size` and set to `Very Small`.

![thunar-prefecences](http://i.imgur.com/Iu1TIEa.png)

#### Notification Area

Go to `Settings Manager` â†’ `Panel` â†’ `Items` tab. Select `Notification Area` item and click on `Edit currently selected item` button. Under `Appearance` set the following options:
